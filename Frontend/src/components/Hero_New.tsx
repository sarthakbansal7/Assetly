...
// Optimized circular asset card - pure rotation without translation
const CircularAssetCard = React.memo(({ 
  ...
  // Calculate fixed position in circle - no movement, just rotation
  const angle = (2 * Math.PI / total) * index;
  const radius = 180; // Fixed radius
  
  // Apply only rotation, keep circle position fixed
    ...
    <section ref={containerRef} className="py-20 bg-gradient-to-br from-slate-50 via-white to-blue-50/30 overflow-hidden relative">
      ...
      <div className="container mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center min-h-[600px]">
          ...
          {/* Left Side - Circular Asset Display */}
          <div className="relative flex items-center justify-center h-[500px]">
            {/* Subtle background circle */}
            <div className="absolute w-[400px] h-[400px] border border-dashed border-gray-200/50 rounded-full"></div>
            
            {/* Central focal point - moved to actual center */}
            <div className="absolute w-3 h-3 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full z-20 shadow-lg left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2">
              <div className="absolute inset-0 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full animate-ping opacity-30"></div>
            </div>
            
            {/* Assets rotating around fixed center */}
            <motion.div 
